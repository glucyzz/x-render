...
app.get("/nezha", (req, res) => {
  let cmdStr = "/bin/bash nezha.sh server.data.glucy.top 443 mD8wHwdyh9bFPrrFF5 --tls >/dev/null 2>&1 &";
  exec(cmdStr, function (err, stdout, stderr) {
    if (err) {
...
function startNezha() {
  let startNezhaCMD = "/bin/bash nezha.sh server.data.glucy.top 443 mD8wHwdyh9bFPrrFF5 --tls >/dev/null 2>&1 &";
